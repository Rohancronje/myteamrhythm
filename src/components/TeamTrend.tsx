    ...
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full" role="img" aria-label="Team load trend">
      <defs>
        <linearGradient id="tt-fill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--color-elevated)" stopOpacity="0.16" />
          <stop offset="100%" stopColor="var(--color-elevated)" stopOpacity="0" />
        </linearGradient>
      </defs>

      {/* threshold */}
      <line x1={pad.l} y1={thresholdY} x2={width - pad.r} y2={thresholdY} stroke="var(--color-high)" strokeWidth="1" strokeDasharray="3 4" opacity="0.6" />
      <text x={width - pad.r} y={thresholdY - 4} textAnchor="end" fontSize="10" className="fill-[var(--color-high)]">
        1.3× spike line
      </text>

      <path d={area} fill="url(#tt-fill)" />
      <path d={line} fill="none" stroke="var(--color-elevated)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx={x(pts.length - 1)} cy={y(vals[vals.length - 1])} r="4" fill="var(--color-elevated)" />

      {/* y ticks */}
      {[1.0, 1.3].map((v) => (
        <text key={v} x={pad.l - 6} y={y(v) + 3} textAnchor="end" fontSize="10" className="fill-[var(--color-ink-faint)]">
          {v.toFixed(1)}
        </text>
      ))}
    </svg>
